...
class MineSweeperAdjacencyCheck {
    ...
    static List<MineSweeperTile> getAdjacentTiles(MineSweeperTile[][] tiles, int row, int col) {
        ...
        tileToReturn.addAll(getAdjacentTiles_recursive(tiles, row, col));
        return tileToReturn;
    }

    /**
     * Get the adjacent tiles of a tile using its id (only the one above, below and next to it,
     * top and bottom corners are not included).
     *
     * @param tiles the list of all tiles
     * @param row   the row number of the tile
     * @param col   the columns of the tile
     * @return Returns a list of all adjacent tiles (four corners not included)
     */
    static List<MineSweeperTile> getAdjacentTiles_recursive(MineSweeperTile[][] tiles, int row,
                                                            int col) {
        List<MineSweeperTile> tileToReturn = new ArrayList<>();
        for (int curRow = 0; curRow < MineSweeperBoard.getNumRows(); curRow++) {
            for (int curCol = 0; curCol < MineSweeperBoard.getNumCols(); curCol++) {
                if (curRow == row && curCol + 1 == col) {
                    tileToReturn.add(tiles[curRow][curCol]);
